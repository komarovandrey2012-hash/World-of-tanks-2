...
from hp_indicator import HPIndicator
...
class Unit:
    ...
    def _create(self):
        self._id = self._canvas.create_image(self._x, self._y, image = skin.get(self._default_image))
    ...
    def _repaint(self):
        ...
        # НЕ НУЖНО здесь вызывать _hp_indicator.update()
        # Это делается в update()
...
class Tank(Unit):
    ...
    def destroy(self):
        ...
        # Скрываем индикатор
        if hasattr(self, '_hp_indicator') and self._hp_indicator:
            self._hp_indicator.hide()
            self._hp_indicator = None  # Убираем ссылку

        # Удаляем изображение танка
        try:
            self._canvas.delete(self._id)
        except Exception:
            pass
    ...
    def update(self):
        ...
        # Добавь ЭТИ строки для обновления индикатора
        if hasattr(self, '_hp_indicator'):
            self._hp_indicator.update()
    ...
    def _take_bon_speed(self):
        if self._turn == 0:
            self._usual_speed += self._usual_speed
            self._unusual_speed += self._unusual_speed
            self._turn += 1
        elif self._turn >= 1:
            self._usual_speed = self._usual_speed
            self._unusual_speed = self._unusual_speed
    ...
    def _set_water_speed(self):
        self._speed = self._unusual_speed
    ...
    def _on_map_collision(self, details):
        if world.WATER in details and len(details) == 1:
            ...
            if world.take(pos['row'], pos['col']) != world.AIR:
                self._take_ammo()
        elif world.Speed_up in details:
            pos = details[world.Speed_up]
            if world.take(pos['row'], pos['col']) != world.AIR:
                self._take_bon_speed()
        else:
            self._undo_move()
            if self._bot:
